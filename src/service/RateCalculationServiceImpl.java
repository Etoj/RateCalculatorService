...
public class RateCalculationServiceImpl implements RateCalculationService {
    ...
    public RateCalculationServiceImpl(TimePointService timePointService, AmountsCalculationService amountsCalculationService, ResidualCalculationService residualCalculationService) {
    ...
    public List<Rate> calculate(InputData inputData) {
        ...
        Rate firstRate = caluclateRate(rateNumber, inputData);
        ...
        for (BigDecimal index = rateNumber.add(BigDecimal.ONE); index.compareTo(inputData.getMonthsDuration()) <= 0; index = index.add(BigDecimal.ONE)) {
            ...
            Rate nextRate = calcualteRate(index, inputData, previousRate);
            rates.add(nextRate);
    ...
    private Rate caluclateRate(BigDecimal rateNumber, InputData inputData) {
    ...
    private Rate calcualteRate(BigDecimal rateNumber, InputData inputData, Rate previousRate) {
        TimePoint timePoint= timePointService.calculate(rateNumber, inputData);
